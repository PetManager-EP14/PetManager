...
class SaleMapperTest {
    ...
    void toDTO_shouldMapEntityToDTO() {
        ...
        assertThat(dto.getCustomerName()).isEqualTo(user.getName()); 
    ...
    void toEntity_shouldMapDTOToEntityIgnoringDetailsAndNotification() {
        ...
        assertThat(sale.getSaleDetails()).isEmpty();
        assertThat(sale.getSaleNotification()).isNull(); 
    }

    // --- 3⃣ Test: SaleDetailDTO -> SaleDetails ---
    @Test
    void toEntity_shouldMapSaleDetailDTOToEntity() {
        SaleDetailDTO dto = new SaleDetailDTO();
        dto.setSaleDetailId(10L); // Fuente: [11]
        dto.setProductId(999L);
        dto.setAmount(BigDecimal.valueOf(3));
        
        SaleDetails entity = mapper.toEntity(dto); // Fuente: [11]
        
        assertThat(entity).isNotNull(); // Fuente: [11]
        assertThat(entity.getSaleDetailId()).isEqualTo(10L); // Fuente: [11]
        assertThat(entity.getProduct()).isNotNull(); // Fuente: [11]
        assertThat(entity.getProduct().getProductId()).isEqualTo(999L); // Fuente: [11]
        assertThat(entity.getAmount()).isEqualTo(BigDecimal.valueOf(3)); // Fuente: [11]
        assertThat(entity.getSale()).isNull(); // Fuente: [11]
        assertThat(entity.getCreatedAt()).isNotNull(); // Fuente: [11]
    }

    // --- 4⃣ Test: SaleDetails -> SaleDetailDTO ---
    @Test
    void toDTO_shouldMapSaleDetailsToDTO() {
        Product product = new Product();
        product.setProductId(888L);
        product.setName("Producto Test");
        product.setPriceSale(BigDecimal.TEN); 
        ...
        assertThat(dto.getProductName()).isEqualTo("Producto Test");
        assertThat(dto.getUnitPrice()).isEqualTo(BigDecimal.TEN);
    }

    // --- 5⃣ Test: Helper map(Long productId) ---
    @Test
    ...
    // --- 6⃣ Test: Helper map(Product product) ---
    @Test
    ...
    // SE ELIMINA el Test 7 (@AfterMapping fillDetailInfo)
    // Esto es para resolver el error de compilación: 
}
